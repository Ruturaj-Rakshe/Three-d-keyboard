...
//This basically means I am taking the group tag's props and adding 2 props
//more to it, and then using it in the forwardRef in the Keyboard function below.
...
export const Keyboard = forwardRef<KeyboardRefs, KeyboardProps>(
  ({ keycapMaterial, knobColor, ...props }, ref) => {
    ...
    const defaultKeyCapMaterial = new THREE.MeshStandardMaterial({
      roughness: 0.7,
      map: keyCapTexture,
    })
    //prop takes priority over default, always. That is why it wasn't working before.
    const keyCapMat = keycapMaterial || defaultKeyCapMaterial
    ...
    const bottomCaseMaterial = new THREE.MeshStandardMaterial({
      color: "#1E548A",
      roughness: 0.4,
    })

    const topCaseMaterial = new THREE.MeshStandardMaterial({
      color: "#dddddd",
      roughness: 0.4,
    })
